...
import { getSocialBoost } from './services/social_booster';
            ...
            >
              ...
              <div className="hidden md:flex gap-8 items-center">
                ...
                <div className="flex-1">
                  ...
                  <h3 className="text-xl font-black text-text mb-3 group-hover:text-accent transition-colors leading-tight drop-shadow-sm">
                    {toUnicodeBold(article.title)}
                  </h3>
                  <p className="text-sm text-text/60 line-clamp-2 leading-relaxed italic mb-4">
                    "{article.summary}"
                  </p>
                  <div className="flex items-center gap-1 text-accent font-black text-[10px] uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all">
                    Read Intelligence <ChevronRight className="w-3 h-3" />
                  </div>
                </div>
...
const AnalysisImage = React.memo(({ src, alt, className = "", rounded = "rounded-[2.5rem]" }: { src?: string, alt: string, className?: string, rounded?: string }) => {
  ...
  // Robust URL resolver
  const getImageUrl = (url?: string, useProxy = true) => {
    ...
    if (trimmedUrl.includes('images.unsplash.com')) {
      ...
      return trimmedUrl.includes('w=') ? trimmedUrl : `${trimmedUrl}&auto=format&fit=crop&q=80&w=1200`;
    }
    
    // Handle Unsplash IDs (numerical strings or specific patterns)
    if (!trimmedUrl.startsWith('http') && trimmedUrl.length > 5 && !trimmedUrl.includes('/') && !trimmedUrl.includes(':')) {
       // Clean the ID - remove 'photo-' if Gemini included it despite instructions
       const cleanId = trimmedUrl.replace(/^photo-/, '');
       return `https://images.unsplash.com/photo-${cleanId}?auto=format&fit=crop&q=80&w=1200`;
    }

    // Handle local paths or supabase storage
    if (trimmedUrl.startsWith('/') || trimmedUrl.includes('localhost') || trimmedUrl.includes('supabase.co')) return trimmedUrl;
    
    // If it's a full URL but not Unsplash
    if (trimmedUrl.startsWith('http')) {
      return useProxy ? `/api/proxy-image?url=${encodeURIComponent(trimmedUrl)}` : trimmedUrl;
    }

    return fallbackImage;
  };

  const imageUrl = !src ? fallbackImage : getImageUrl(src, !retryWithoutProxy);
  ...
  return (
    <div className={`relative w-full aspect-video overflow-hidden bg-slate-200 dark:bg-slate-800 ${rounded} border border-text/10 shadow-lg ${className}`}>
      ...
      <img
        ...
        onError={(e) => {
          if (!retryWithoutProxy && String(src).startsWith('http')) {
            console.log(`[IMAGE-RETRY] Retrying without proxy for: ${src}`);
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_400px] gap-8 xl:gap-12 items-start">
        ...
        <div className="space-y-8 md:space-y-12">
          ...
          <div className="space-y-6 md:space-y-8">
            ...
            <h1 className="text-3xl md:text-5xl xl:text-6xl font-black leading-[1.05] text-text tracking-tight max-w-full xl:max-w-[95%] drop-shadow-md">
              {toUnicodeBold(article.title)}
            </h1>
          ...
          <div className="flex flex-wrap gap-4 items-center mt-12 md:mt-16 pt-8 border-t border-white/5">
            ...
            <button 
              onClick={() => {
                const boost = getSocialBoost(article.title, article.summary, article.category);
                let shareText = `📢 SaaS INTELLIGENCE: ${article.title}\n\n${article.summary}\n\n💡 ${boost.cta}\n\nRead more on SaaS Sentinel: ${window.location.origin}/article/${article.id || ''}`;
                if (boost.mentions.length > 0) {
                  shareText += `\n\nCc: ${boost.mentions.join(' ')}`;
                }
                if (boost.tags.length > 0) {
                  shareText += `\n\n${boost.tags.join(' ')}`;
                }
                      ...
                      <form onSubmit={handleSubscribe} className="space-y-4">
                        ...
                        {!subscribeSuccess && (
                          <button
                            ...
                            className="w-full py-3 rounded-2xl text-[10px] uppercase tracking-widest font-black text-text/40 hover:text-text hover:bg-white/5 transition-all flex items-center justify-center gap-2"
                          >
