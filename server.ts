...
app.get(["/sitemap.xml", "/api/sitemap.xml"], async (req, res) => {
  try {
    ...
    const { data: articles, error } = await supabase
      .from("news_articles")
      .select("id, slug, updated_at, created_at, image_url, title, category")
...
app.get(['/article/:slugOrId', '/news/:slugOrId', '/about', '/privacy', '/archive', '/', '/index.html'], async (req, res, next) => {
  ...
  try {
    ...
    // Replace default head metas with dynamic metas
    let ldJsonString = "";
    if (ldJsonObj) {
    ...
    // Remove static default title and any canonical tags from the template
    ...
    // Inject rich pre-rendered HTML content inside #root for crawlers
    if (bodyHtml) {
    ...
    console.error("Dynamic Metadata SSR Handler Crashed in server.ts:", err);
