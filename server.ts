...
app.use(async (req, res, next) => {
  ...
  // 1. PARANOID BOT DETECTION (v31 - The Ghost)
  const botSignatures = [
  ...
  const isApiSocialPath = req.path.includes('/api/social/') || req.path.includes('/api/v31/share/');
  ...
  // 2. INFRASTRUCTURE BYPASS: Intercept cookie checks for bots (v31 Ghost)
  if (isCookieCheck && (isBot || isApiSocialPath)) {
    try {
      const decodedReturnUrl = decodeURIComponent(decodeURIComponent(returnUrl || ""));
      console.log(`[BYPASS-V31] Cookie-check Ghost Rescue! ReturnURL: ${decodedReturnUrl}`);
      
      if (articleId) {
        // BINARY IMAGE RESCUE
        const isImageBinaryReq = decodedReturnUrl.includes('og-image.jpg') || decodedReturnUrl.includes('static-preview') || decodedReturnUrl.endsWith('.jpg') || decodedReturnUrl.endsWith('.png');
        if (isImageBinaryReq) {
          console.log(`[BYPASS-V31] Image Rescue: ${articleId}`);
  ...
  if (isRealBrowser && (isApiSocialPath || isBotPath) && articleId) {
    console.log(`[HUMAN-REDIRECT-V31] Article: ${articleId}`);
    ...
    try {
      ...
      if (article) {
        ...
        if (article.image_url) {
          ...
          // NEW CLEAN STATIC PROXY URL (v31 reset)
          ogImage = `${cleanBase}/api/static-preview/${articleId}/og-image.jpg?ref=v31`;
        }
      }
    } catch (e) {}
  }
  ...
  // For bots, we want og:url to point to the scrappable portal path (v31)
  const botFriendlyUrl = articleId ? `${cleanBase}/api/v31/share/${articleId}/preview.jpg` : `${cleanBase}${req.originalUrl}`;
  ...
  if (isBot) {
    ...
    console.log(`[BOT-FINAL-V31] Responding: ${req.path} | Article: ${articleId}`);
