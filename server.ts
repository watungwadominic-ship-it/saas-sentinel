...
app.use(async (req, res, next) => {
  ...
  // 1. PARANOID BOT DETECTION (v34 - The Final Wall)
  const botSignatures = [
  ...
  const isV34Path = req.path.includes('/v34/');
  const isInsightPath = req.path.includes('/insights/');
  const isBotPath = isV34Path || isInsightPath || req.path.includes('.well-known') || req.path.includes('/portal/') || req.path.includes('/api/v');
  ...
  // 0. BINARY IMMUNITY
  const isApiSocialPath = (req.path.includes('/api/v') || req.path.includes('/api/social')) && (req.path.includes('/share/') || req.path.includes('/social/') || req.path.includes('/preview'));
  const isImageRequest = !isInsightPath && !isV34Path && !isApiSocialPath && (req.path.includes('/api/static-preview') || req.path.includes('/api/proxy-image') || req.path.endsWith('.jpg') || req.path.endsWith('.png'));
  ...
  // THE MASTER BOT FLAG (v34)
  const isBotRaw = isBotUA || isLinkedInHeaders || isBotPath || isBotQuery || forceBot;
  
  // Rescue bots trapped in cookie checks (v34)
  const isBotInRescue = isCookieCheck && (!hasBrowserSign || isBotUA || isLinkedInHeaders || (returnUrl && (returnUrl.includes('v34/') || returnUrl.includes('insights/') || returnUrl.includes('api/') || returnUrl.includes('portal/') || returnUrl.includes('ref=v'))));
  ...
  if (isCookieCheck && returnUrl) {
    try {
      ...
      const idMatch = decodedReturnUrl.match(/v34\/(\d+)/i) ||
                      decodedReturnUrl.match(/insights\/(\d+)/i) || 
                      decodedReturnUrl.match(/share\/(\d+)/i) || 
                      decodedReturnUrl.match(/social\/(\d+)/i) || 
                      decodedReturnUrl.match(/static-preview\/(\d+)/i) ||
  ...
  if (!articleId) {
    const idMatch = req.path.match(/v34\/(\d+)/i) ||
                    req.path.match(/insights\/(\d+)/i) ||
                    req.path.match(/share\/(\d+)/i) ||
                    req.path.match(/social\/(\d+)/i) ||
                    req.path.match(/static-preview\/(\d+)/i) ||
                    req.path.match(/portal\/news\/(\d+)/i) ||
                    req.path.match(/\/(?:article|news|og-article-)\/([^\/?#.]+)/i);
    if (idMatch) articleId = idMatch[1];
  }

  // 2. INFRASTRUCTURE BYPASS: Intercept cookie checks for bots (v34 Absolute)
  if (isCookieCheck && (isBot || isV34Path || isInsightPath)) {
    try {
      const decodedReturnUrl = decodeURIComponent(decodeURIComponent(returnUrl || ""));
      console.log(`[BYPASS-V34] Cookie-check Rescue! ReturnURL: ${decodedReturnUrl}`);
      
      if (articleId) {
        // BINARY IMAGE RESCUE
        const isMetadataPath = decodedReturnUrl.includes('/share/') || decodedReturnUrl.includes('/social/') || decodedReturnUrl.includes('/portal/') || decodedReturnUrl.includes('/insights/') || decodedReturnUrl.includes('/v34/');
        const isImageBinaryReq = (decodedReturnUrl.includes('og-image.jpg') || decodedReturnUrl.includes('static-preview')) && !isMetadataPath;
        
        if (isImageBinaryReq) {
          console.log(`[BYPASS-V34] Image Binary Rescue: ${articleId}`);
  ...
  if (isRealBrowser && (isV34Path || isInsightPath || isBotPath) && articleId) {
    console.log(`[HUMAN-REDIRECT-V34] Article: ${articleId}`);
    ...
    try {
      ...
      if (article) {
        ...
        if (article.image_url) {
          // V34: USE ORIGINAL IMAGE URL DIRECTLY IF IT'S HTTPS
          if (article.image_url.startsWith('https://')) {
            ...
            ogImage = `${cleanBase}/api/static-preview/${articleId}/og-image.jpg?ref=v34`;
          }
        }
      }
    } catch (e) {}
  ...
  // V34: THE SACRED OG:URL
  // This must match exactly what we share.
  const canonicalUrl = articleId ? `${cleanBase}/v34/${articleId}` : `${cleanBase}${req.originalUrl}`;
...
<link rel="canonical" href="${canonicalUrl}" />`;
  ...
  // 5. BOT RESPONSE (v34 Absolute Exit)
  if (isBot) {
    ...
    console.log(`[BOT-FINAL-V34] Responding: ${req.path} | Article: ${articleId}`);
    res.setHeader('Content-Type', 'text/html; charset=utf-8');
    res.setHeader('Cache-Control', 'public, max-age=3600');
    res.setHeader('X-SaaS-Sentinel-Core', 'v34');
